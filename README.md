...
At the bottom of the sidebar, there's two buttons: Upon clicking the **self-explantory "Download" button**, you'll be presented with a choice of available formats: PNG, JPEG, and SVG, the latter of which is [intended for pen plotting](https://github.com/doersino/uji/issues/3). The adjacent **"Share" button reveals a shareable URL that encodes your options**, which comes in real handy if you want to show a thing you've made with UJI to someone else. (If you press <kbd>alt</kbd> while clicking either button, the default action is performed in each case – PNG download *slash* URL-to-clipboard-copying.)
...
* The possibility to export a minimal, stand-alone JavaScript file with all `opts.*` references in the render loop substituted? That might help folks adapt their results for, say, pen plotting (although SVG export should cover that use case reasonably well). Alternatively, the option to export the computed line segments as JSON (or G-code).
